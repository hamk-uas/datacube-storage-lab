...
See the last link also for interesting ideas for improving SAFE and COG patch load times. The fact that we are aiming at loading patch time series makes those ideas less useful for us, so it is unlikely that we will investigate them, considering how much faster Zarr and zipped Zarr already are.
